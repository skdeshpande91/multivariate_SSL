...
# Identify the points that have same support as the terminal point
# If it does, color the point blue, give it pch = 3 (+)
# If it does not, color the point black, give it pch = 16 (filled circle)
# If it is unstable, color the point red, give it pch = 4 (x)
...
#  }
#}
plot_support <- function(fit_mSSL_dpe){
  
  lambda0 <- fit_mSSL_dpe$lambda0
  xi0 <- fit_mSSL_dpe$xi0
  L <- length(lambda0)
  supp_B_final <- which(fit_mSSL_dpe$B != 0)
  supp_Omega_final <- which(fit_mSSL_dpe$Omega != 0)
  ...
  for(s in 1:L){
    for(t in 1:L){
      index <- (s - 1) + L*(t-1) + 1
      supp_B <- which(fit_mSSL_dpe$B_path[,,index] != 0)
      supp_Omega <- which(fit_mSSL_dpe$Omega_path[,,index] != 0)
      
      if(fit_mSSL_dpe$early_term[index] == 1){
        points(lambda0[s], xi0[t], col = 'red', pch = 4)
      }
      if(identical(supp_B, supp_B_final) & identical(supp_Omega, supp_Omega_final) & fit_mSSL_dpe$early_term[index] == 0){
        points(lambda0[s],xi0[t], col = 'blue', pch = 3)
      } else if(fit_mSSL_dpe$early_term[index] == 0){
        points(lambda0[s],xi0[t], col = 'black', pch = 16)
      } else if(fit_mSSL_dpe$early_term[index] == 1){
        points(lambda0[s],xi0[t], col = 'red', pch = 4)
      }
    }
  }
